...
const shared_1 = require("@digital-inspect/shared");
const inspectionsRoutes = new hono_1.Hono();
inspectionsRoutes.use('*', auth_1.authMiddleware);
// ==========================================
// TASKS (Jadwal)
// ==========================================
// Delete a task (Admin/Manager only)
inspectionsRoutes.delete('/tasks/:id', (0, auth_1.roleGuard)(['SUPER_ADMIN', 'MANAGER']), async (c) => {
    try {
        const { id } = c.req.param();
        // Check if this task has an associated forklift report
        const flReport = await (0, db_1.default) `SELECT id FROM forklift_inspections WHERE task_id = ${id}`;
        if (flReport.length > 0) {
            await (0, db_1.default) `DELETE FROM forklift_inspection_scores WHERE inspection_id = ${flReport[0].id}`;
            await (0, db_1.default) `DELETE FROM forklift_inspections WHERE id = ${flReport[0].id}`;
        }
        // Check if this task has an associated battery report
        const batReport = await (0, db_1.default) `SELECT id FROM battery_service_reports WHERE task_id = ${id}`;
        if (batReport.length > 0) {
            await (0, db_1.default) `DELETE FROM battery_service_reports WHERE id = ${batReport[0].id}`;
        }
        // Now delete the task
        await (0, db_1.default) `DELETE FROM inspection_tasks WHERE id = ${id}`;
...
inspectionsRoutes.get('/tasks', (0, auth_1.roleGuard)(['SUPER_ADMIN', 'MANAGER', 'DIRECTOR']), async (c) => {
    try {
        const tasks = await (0, db_1.default) `
      SELECT t.*, 
...
inspectionsRoutes.post('/schedule', (0, auth_1.roleGuard)(['SUPER_ADMIN', 'MANAGER']), async (c) => {
    try {
        ...
        const newTask = await (0, db_1.default) `
...
inspectionsRoutes.get('/asset/:id', async (c) => {
    try {
        ...
        if (type === 'battery') {
            const reports = await (0, db_1.default) `
...
inspectionsRoutes.post('/forklift', async (c) => {
    try {
        ...
        const dbItems = await (0, db_1.default) `SELECT id, is_critical FROM inspection_items WHERE id IN ${(0, db_1.default)(itemIds)}`;
        ...
        const health_percentage = (0, shared_1.calculateHealthScore)(totalScore, totalItems);
        const health_status = (0, shared_1.determineHealthStatus)(health_percentage, hasCriticalIssue);
        // 2. Start Transaction
        ...
        VALUES (${task_id || null}, ${forklift_id}, ${user.userId}, ${totalScore}, ${totalItems}, ${health_percentage}, ${health_status}, ${notes || null}, ${db_1.default.json(finalAdditionalData)}, NOW())
        RETURNING id
      `;
...
inspectionsRoutes.post('/battery', async (c) => {
    try {
        ...
        VALUES (${task_id || null}, ${battery_id}, ${user.userId}, ${voltage_reading || null}, ${db_1.default.json(finalReportData)}, NOW())
        RETURNING id
      `;
...
inspectionsRoutes.get('/forklift/:id', async (c) => {
    try {
        const { id } = c.req.param();
        const report = await (0, db_1.default) `
      ...
      WHERE fi.id = ${id}
    `;
        const scores = await (0, db_1.default) `SELECT fis.score, fis.photo_url, i.name as item_name, c.name as category_name FROM forklift_inspection_scores fis JOIN inspection_items i ON fis.item_id = i.id JOIN inspection_categories c ON i.category_id = c.id WHERE fis.inspection_id = ${id}`;
...
exports.default = inspectionsRoutes;
