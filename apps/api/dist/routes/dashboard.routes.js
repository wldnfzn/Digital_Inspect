...
const dashboardRoutes = new hono_1.Hono();
dashboardRoutes.use('*', auth_1.authMiddleware);
dashboardRoutes.use('*', (0, auth_1.roleGuard)(['SUPER_ADMIN', 'DIRECTOR', 'MANAGER']));
dashboardRoutes.get('/stats', async (c) => {
    try {
        const month = c.req.query('month');
        // For queries that don't need month filtering (overall counts)
        const forkliftCount = await (0, db_1.default) `SELECT count(*) FROM forklifts`;
        const batteryCount = await (0, db_1.default) `SELECT count(*) FROM batteries`;
        const ongoingTasks = await (0, db_1.default) `
      SELECT count(*) FROM inspection_tasks 
      WHERE status IN ('SCHEDULED', 'IN_PROGRESS')
    `;
        // Dynamic month filtering for completed tasks
        let completedCondition = (0, db_1.default) `completed_at >= date_trunc('month', CURRENT_DATE)`;
        let monthFilterCondition = (0, db_1.default) `1=1`; // Default no filter if no month provided, wait no, let's filter if month is provided
        if (month) {
            completedCondition = (0, db_1.default) `to_char(completed_at, 'YYYY-MM') = ${month}`;
            monthFilterCondition = (0, db_1.default) `to_char(completed_at, 'YYYY-MM') = ${month}`;
        }
        const completedForklifts = await (0, db_1.default) `SELECT count(*) FROM forklift_inspections WHERE ${completedCondition}`;
        const completedBatteries = await (0, db_1.default) `SELECT count(*) FROM battery_service_reports WHERE ${completedCondition}`;
        // Health Status
        const goodStatus = await (0, db_1.default) `SELECT count(*) FROM forklift_inspections WHERE health_status = 'HEALTHY' AND ${monthFilterCondition}`;
        const attentionStatus = await (0, db_1.default) `SELECT count(*) FROM forklift_inspections WHERE health_status = 'ATTENTION' AND ${monthFilterCondition}`;
        const criticalStatus = await (0, db_1.default) `SELECT count(*) FROM forklift_inspections WHERE health_status = 'CRITICAL' AND ${monthFilterCondition}`;
        // Avg Fleet Health (Overall)
        const avgHealth = await (0, db_1.default) `SELECT avg(health_percentage) as avg FROM forklift_inspections WHERE ${monthFilterCondition}`;
        // NEW: Avg Health Per Asset (Forklifts)
        const avgHealthPerAsset = await (0, db_1.default) `
    ...
    `;
        ...
        const activityInspections = await (0, db_1.default) `
    ...
    `;
        ...
        const mechanicInspections = await (0, db_1.default) `
    ...
    `;
        ...
        const recentForklifts = await (0, db_1.default) `
    ...
    `;
        const recentBatteries = await (0, db_1.default) `
    ...
    `;
        ...
        const criticalAlerts = await (0, db_1.default) `
      SELECT 
        f.asset_code, 
      ...
      WHERE fis.score = 1 AND ${month ? (0, db_1.default) `to_char(fi.completed_at, 'YYYY-MM') = ${month}` : (0, db_1.default) `1=1`}
      ORDER BY fi.completed_at DESC 
      LIMIT 50
    `;
        // Customer Summary
        const customerSummary = await (0, db_1.default) `
      SELECT 
        c.name, 
